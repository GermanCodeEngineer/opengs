...
@onready var map: Node = get_node_or_null("../Map")
...
var camera_rotation_acceleration_speed = 1.5
var camera_velocity_half_life_rotation = camera_velocity_half_life_xy

@export_range(0,10,0.1) var camera_rotation_speed:float = 0.20 # TODO: remove
@export_range(0,20,1) var camera_base_rotation_speed:float = 6 # TODO: remove?
...
# Bounds
var camera_minimum: Vector3 = Vector3(-460.0, -303.0, -1000.0)
...
func _set_x_rotation(rot: float) -> void:
	camera_socket.rotation.x = rot
func _set_z_rotation(rot: float) -> void:
	#camera.rotation.z = rot
	rotation.y = rot
func _get_rotation() -> Vector3:
	return Vector3(camera_socket.rotation.x, rotation.y, 0)
...
func _process(delta:float) -> void:
	...
	if label_camera_position:
		label_camera_position.text = str(_round_vec(position)) + " P|R " + str(_round_vec(_get_rotation()))
...
func camera_base_move(delta:float) -> void:
	...
	# Calculate acceleration based on camera z-rotation
	var jerk_direction: Vector3 = Vector3.ZERO

	#"""
	if Input.is_action_pressed("camera_forward"): jerk_direction -= transform.basis.z
	if Input.is_action_pressed("camera_backward"): jerk_direction += transform.basis.z
	if Input.is_action_pressed("camera_right"): jerk_direction += transform.basis.x
	if Input.is_action_pressed("camera_left"): jerk_direction -= transform.basis.x

	if Input.is_action_pressed("camera_up"): jerk_direction += transform.basis.y
	if Input.is_action_pressed("camera_down"): jerk_direction -= transform.basis.y
	jerk_direction = jerk_direction.normalized()
	#"""
	#velocity_direction.x += camera_touchpad_move.x
	#velocity_direction.z += camera_touchpad_move.y
	#position += jerk_direction.normalized() * 500  * delta


	#TEMP:print("Jerk Dir", jerk_direction)
	frame_acceleration += jerk_direction

	# Scale acceleration by speed and delta
	# Move faster when camera is further away from map
	#TEMP:print("Frame Acc unscaled", frame_acceleration)
	var z_multiplier = max(position.z, camera_minimum.z)
	...
	#TEMP:print("Dampend velocity", camera_velocity)
	#TEMP:print()
...
func camera_base_rotate(delta:float) -> void:
	...
	# Calculate rotation acceleration from input (yaw only)
	if Input.is_action_pressed("camera_rotate_right"):
		frame_rotation_acceleration.x -= 1.0
	if Input.is_action_pressed("camera_rotate_left"):
		frame_rotation_acceleration.x += 1.0

	# Scale acceleration and integrate into angular velocity
	frame_rotation_acceleration.x *= camera_rotation_acceleration_speed
	frame_rotation_acceleration *= delta
	camera_rotation_velocity += frame_rotation_acceleration
	frame_rotation_acceleration = Vector2.ZERO

	# Integrate angular velocity into rotation
	_set_z_rotation(_get_rotation().y + camera_rotation_velocity.x)

	# Dampen angular velocity (half-life based)
	var damp_rotation = exp(NLOG2 * delta / camera_velocity_half_life_rotation)
	camera_rotation_velocity *= damp_rotation
...
func camera_automatic_pan(delta:float) -> void:
	...
	var zoom_factor:float = position.z * 0.1
